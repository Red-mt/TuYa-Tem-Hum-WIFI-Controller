...
*********************************************************************************/
/**
 * @file    protocol.c @author  涂鸦综合协议开发组
 * @version v2.5.6
 * @date    2020.12.16
 * @brief
 *                       *******非常重要，一定要看哦！！！********
...
{int TEM, HUM;
  ...
  sht31.begin(0x44);//SHT地址0x44
      ...
      Serial.println(sht31.readHumidity());      //串口显示湿度
      Serial.print("Temperature (oC): ");
      Serial.println(sht31.readTemperature());     //串口显示温度
  ...
  relay = digitalRead(10);//继电器状态读取
  //  #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
  mcu_dp_value_update(DPID_TEMP_CURRENT, TEM); //VALUE型数据上报;
  mcu_dp_value_update(DPID_HUMIDITY_VALUE, HUM); //VALUE型数据上报;
  mcu_dp_value_update(DPID_MAXHUM_SET, HUMUPPR.a); //VALUE型数据上报;
  mcu_dp_value_update(DPID_MINIHUM_SET, HUMLOWER.a); //VALUE型数据上报;
  mcu_dp_enum_update(DPID_HUM_ALARM, 1); //枚举型数据上报;  loweralarm, upperalarm, cancel
  mcu_dp_bool_update(DPID_SWITCH, relay); //BOOL型数据上报;
  ...
  mcu_dp_enum_update(DPID_HUM_ALARM,当前湿度报警); //枚举型数据上报;  loweralarm, upperalarm, cancel
...
#define relay 10
  ...
  if (switch_1 == 0) {
    digitalWrite(relay, LOW);//开关关
  } else {
    digitalWrite(relay, HIGH);//开关开
  }
